    ...
    public static Object download(String url,String path) {
        ...
        try {
            ...
            if (resp.isOK()) {
                ...
                File tmpfile = File.createTempFile("pic", fileext,new File(path));
    ...
    public static Object downloadRemote(String url,String remote) {
    	...
    	try {
    		...
    		if (resp.isOK()) {
    			...
    			//1 返回相对路径
    			String path = filename + fileext; //png
    			//给请求路径 URL
    			remote = remote + path;
    			//此对象可以发送图片
    			WebResource resource = client.resource(remote);
    			resource.put(String.class);
    			
    			File tmpfile = new File(remote);
    			return tmpfile;
    		}
